---
title: 'The ONE AI Prompt That 10x Your Traffic Analysis Overnight'
...
<prompt>How has the {PRODUCT/SERVICE} conversion rate on {WEBSITE NAME} been altered during the previous {TIME PERIOD}? Which factors could potentially account for these alterations?</prompt>
...
1. **Quantify the change** – It calculates the difference in conversion rate between two comparable periods (e.g., month‑over‑month, quarter‑over‑quarter).  
...
1. Restate the change (so you know it understood).  
2. List possible factors grouped by category (technical, marketing, external).  
3. Suggest next steps (e.g., “Check GA4’s ‘User Explorer’ for Safari‑only traffic” or “Run a landing‑page heatmap”).
